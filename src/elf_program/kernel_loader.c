...
EFI_STATUS load_kernel(EFI_HANDLE image_handle, kernel_main_ptr *kernel_main) {
    ...
    EFI_PHYSICAL_ADDRESS kernel_address = min_virtual_address;
    status = BS->AllocatePages(AllocateAddress, EfiLoaderData, num_pages, &kernel_address);
    ASSERT_NO_EFI_ERROR();
    ...
    for(int i = 0; i < elf_header->e_phnum; i++) {
        ...
        if(ph->p_flags & PT_LOAD) {
            memcpy((void*)ph->p_vaddr, kernel_buffer + ph->p_offset, ph->p_filesz);
            memset((void*)(ph->p_vaddr + ph->p_filesz), 0, ph->p_memsz - ph->p_filesz);
        }
    }

    *kernel_main = (kernel_main_ptr) elf_header->e_entry;
